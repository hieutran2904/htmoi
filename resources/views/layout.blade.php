...
<body class="home blog halimthemes halimmovies" data-masonry="">
        ...
        <div class="container">
            <div class="row" id="headwrap">
                <div class="col-md-3 col-sm-6 slogan">
                    <p class="">
                        <a class="logo" src="{{ route('homepage') }}" title="phim hay ">
                            <img width="65px;" src="{{ asset('imgs/logo.jpg') }}" alt="">
                        </a>
                    </p>
                </div>
                        ...
                        <div class="col-xs-12">
                            ...
                            <div class="form-inline">
                                <div class="input-group col-xs-12">
                                    <form action="{{ route('search-film') }}" method="GET">
                                        <div class="row text-center">
                                            <div class="col-md-9" style="padding: 0;">
                                                <input id="search-film" style="width:100%;" type="text"
                                                    name="search-film" class="form-control" placeholder="Tìm kiếm..."
                                                    autocomplete="off" required>
                                                {{-- <i class="animate-spin hl-spin4 hidden"></i> --}}
                                            </div>
                                            <div class="col-md-3" style="padding: 0;">
                                                <button style="width:100%;"
                                                    class="form-control btn btn-primary col-xs-3">Tìm kiếm</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                            ...
                            {{-- </form> --}}
                            {{-- <ul class="ui-autocomplete hidden results-film"></ul> --}}
                            <ul class="list-group" id="results-film"></ul>
                        </div>
                    </div>
                </div>
                                ...
                                <ul role="menu" class=" dropdown-menu">
                                    @foreach ($genre as $item)
                                        <li><a title="{{ $item->title }}"
                                                href="{{ route('genre', ['slug' => $item->slug]) }}">{{ $item->title }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </li>
                            <li class="mega dropdown">
                                ...
                                <ul role="menu" class=" dropdown-menu">
                                    @foreach ($country as $item)
                                        <li><a title="{{ $item->title }}"
                                                href="{{ route('country', ['slug' => $item->slug]) }}">{{ $item->title }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </li>
            ...
            <div class="row container">
                <div class="widget about col-xs-12 col-sm-4 col-md-4">
                    <div class="footer-logo">
                        <img class="img-responsive" src="{{ asset('imgs/logo.jpg') }}" alt="Hieu Tran" />
                    </div>
    ...
    <script async defer crossorigin="anonymous" src="https://connect.facebook.net/vi_VN/sdk.js#xfbml=1&version=v17.0"
        nonce="GiEBPk4E"></script>
    {{-- end comment --}}
    <script type='text/javascript' src='{{ asset('js/halimtheme-core.min.js?ver=1626273138') }}' id='halim-init-js'>
    </script>
    <script type='text/javascript'>
        $(document).ready(function() {
            $('#show_default').load('{{ route('filter-top-view') }}?value=0')
        })
        $('.filter-sidebar').click(function() {
            $('.show_default').hide();
            var href = $(this).attr('href');
            if (href == '#day') var value = 0;
            if (href == '#week') var value = 1;
            if (href == '#month') var value = 2;
            $.ajax({
                url: "{{ route('filter-top-view') }}",
                method: "GET",
                data: {
                    value: value,
                },
        ...
        $(document).ready(function() {
            $('#search-film').keyup(function() {
                ...
                if (search != '') {
                    ...
                    $.getJSON('/json/movies.json', function(data) {
                        $.each(data, function(key, value) {
                            if (value.title.search(expression) != -1 || value.description
                                .search(expression) != -1) {
                                $('#results-film').css('display', 'inherit');
                                $('#results-film').append(
                    ...
                    $('#results-film').css('display', 'none');
                }
            });
            $('#results-film').on('click', 'li', function() {
                var click_text = $(this).text().split('|');
                $('#search-film').val($.trim(click_text[0]));
                $('#results-film').html('');
            })
        });
    </script>
